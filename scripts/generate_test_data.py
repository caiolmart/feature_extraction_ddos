...
logging.basicConfig(filename='logs/generate_test_data.log', 
                    format='%(asctime)s %(message)s', 
                    level=logging.DEBUG)
logging.info('Starting program.')

pcap_num = 0
capture = rdpcap('data/raw/pcap/03-11/SAT-03-11-2018_0')
idx_pcap = 0
...
for filename in files:
    ...
    data = pd.read_csv(f'data/raw/csv/03-11/{filename}')
    ...
    outfile = f'data/interim/test/flow_features_{filename}'
    f = open(outfile,'w')
    ...
    while idx < len_data:
        ...
        if idx_pcap < len_pcap:
            ...
            logging.info(f'memory_twotup has {len(afg.memory_twotup)} flows.')
            logging.info(f'memory_fivetup has {len(afg.memory_fivetup)} flows.')
            capture = rdpcap(f'data/raw/pcap/03-11/SAT-03-11-2018_0{pcap_num}')
            idx_pcap = 0
            len_pcap = len(capture)
            packet = capture[idx_pcap]
        afg.update(packet)
